...
def get_basic_part() -> dict:
    ...
    rules_dict['rules'] = [x.replace(',Proxy',',PROXY') for x in rules_dict['rules']]
...
def get_rules_part(filename: str = 'hackl0us_rules.yaml') -> dict:
...
def get_rule_providers_part(filename: str = 'hackl0us_providers.yaml') -> dict:
    with open('./clash_rule/'+filename, 'r') as f:
        my_config = yaml.safe_load(f)
        f.close()
    return my_config
...
my_clash_config.update(get_rule_providers_part(
    'loyalsoldier_providers_manual.yaml'))
my_clash_config.update(get_rules_part('loyalsoldier_rules_manual.yaml'))
